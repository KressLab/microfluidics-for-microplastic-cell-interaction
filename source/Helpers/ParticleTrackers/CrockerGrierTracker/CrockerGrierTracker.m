classdef CrockerGrierTracker<handle
    % A convenience interface for the crocker grier tracker (see
    % obj.track())...
    ...
    methods(Access=public)
        function obj=CrockerGrierTracker()
        ...
        % pos:
        %    an array listing the scrambled coordinates and data
        %    of the different particles at different times, such that:
        %    positionlist(0:d,*): contains the d coordinates for all the particles
        %    in the given frame
        function setPositionsInNewFrame(obj,pos)
            obj.trackingDone=false;
            obj.frameCount=obj.frameCount+1;
            new=[pos,repmat(obj.frameCount,size(pos,1),1)];
            if isempty(obj.inputXYZT)
                obj.inputXYZT=new;
            else
                obj.inputXYZT=[obj.inputXYZT;new];
            end
        ...
        % maxdisp:
        %     an estimate of the maximum distance that a particle
        %     would move in a single time interval.(see Restrictions)
        % mem:
        %     this is the number of time steps that a particle can be
        ...
        function setSettings(obj,maxdisp,mem,minTrackLength)
            obj.maxdisp=maxdisp;
            obj.mem=mem;
        ...
        function count=getParticleCount(obj)
            obj.track();
            count = length(unique(obj.outputXYZTI(:,obj.dim+2)));
        end
        ...
        function [positions,ids]=getTrackCellByFrames(obj)
            ...
            for frameId=1:obj.getFrameCount()
                allInFrame=obj.outputXYZTI(obj.outputXYZTI(:,obj.dim+1)==frameId,:);
                positions{frameId,1}=allInFrame(:,1:obj.dim);
                ids{frameId,1}=allInFrame(:,obj.dim+2);
            end
        end
        
        function outputXYZTI=getOutputXYZTI(obj)
            obj.track();
            outputXYZTI=obj.outputXYZTI;
        end
        
        function trs=getTrackStruct(obj)
            obj.logger.warn('This function might be buggy...check this before use');
            obj.track();
            %creates a handy strucutre for manipulating datae from track.m
            % which minimizes find() calls
            % out:  trs(t).r: gives xy(z) locations of each id at each time, if
            % particle not present then xyz given as NaN
            ti=min(obj.outputXYZTI(:,obj.dim+1));

            %make first time 1
            if ti~=1
                obj.outputXYZTI(:,obj.dim+1)=obj.outputXYZTI(:,obj.dim+1)-ti+1;
            end

            %find all particle ids
            particleCount = obj.getParticleCount();

            trs=repmat(struct('r',NaN(particleCount,obj.dim)),1,max(obj.outputXYZTI(:,obj.dim+1)));
            for frameId=unique(obj.outputXYZTI(:,obj.dim+1))'
                obj.logger.trace('Restructuring frame ',frameId,'/',obj.frameCount);
                theFrameParticlePos=obj.outputXYZTI(obj.outputXYZTI(:,obj.dim+1)==frameId,[1:obj.dim,obj.dim+2]);
                trs(frameId).r(theFrameParticlePos(:,3),:)=theFrameParticlePos(:,1:obj.dim);
            end
        end
    end
    ...
    methods(Access=private)
        function track(obj)
            ...
            % ;  9/97 JCC added 'obj.minTrackLength' keyword to improve memory efficiency
            ...
            st = t(2:end) - st(2:end);
            if  sum(st(st < 0)) ~= 0
                obj.logger.fatal('The time vectors is not in order')
            end
            ...
            z = z +1;
            if isempty(w)
                obj.logger.fatal('All positions are at the same time... go back!')
            end
            ...
            tracks = [1,tracks',length(t)];
            ngood = tracks(2) - tracks(1) + 1;
            eyes = 1:ngood;
            pos = obj.inputXYZT(eyes,1:obj.dim);
            istart = 2;
            n = ngood;
            ...
            maxdisq = obj.maxdisp^2;
            
            % John calls this the setup for "fancy code" ???
            notnsqrd = (sqrt(n*ngood) > 200) & (obj.dim < 7);
            notnsqrd = notnsqrd(1);
            
            if notnsqrd
                %;   construct the vertices of a 3x3x3... d-dimensional hypercube
                cube = zeros(3^obj.dim,obj.dim);
                for d=0:obj.dim-1
                    numb = 0;
                    for j=0:(3^d):(3^obj.dim)-1
                        cube(j+1:j+(3^(d)),d+1) = numb;
                        numb = mod(numb+1,3);
                    end
                end
                %   calculate a blocksize which may be greater than obj.maxdisp, but which
                %   keeps nblocks reasonably small.
                
                volume = 1;
                for d = 0:obj.dim-1
                    minn = min(obj.inputXYZT(w,d+1));
                    maxx = max(obj.inputXYZT(w,d+1));
                    volume = volume * (maxx-minn);
                end
                blocksize = max( [obj.maxdisp,((volume)/(20*ngood))^(1.0/obj.dim)] );
            end
            %   Start the main loop over the frames.
            for i=istart:z
                ispan = mod(i-1,zspan)+1;
                % get new particle positions
                m = tracks(i+1) - tracks(i);
                tracks(i);
                eyes = 1:m;
                eyes = eyes + tracks(i);
                if m > 0
                    xyi = obj.inputXYZT(eyes,1:obj.dim);
                    found = zeros(m,1);
                    ...
                    if notnsqrd
                        ...
                        ntrack = length(wh);
                        if ntrack == 0
                            obj.logger.warn('There are no valid particles to track idiot!');
                            break
                        end
                        xmat = zeros(ntrack,m);
                        count = 0;
                            ...
                            if nnew > 5
                                ...
                                for ii =1:nnew
                                    rnsteps = rnsteps * length( find(bonds(:,2) == ...
                                        unew(ii)));
                                    if rnsteps > 5.e+4
                                        obj.logger.warn('Difficult combinatorics encountered.');
                                    end
                                    if rnsteps > 2.e+8
                                        obj.logger.fatal('Excessive Combinitorics you FOOL LOOK WHAT YOU HAVE DONE TO ME!!!');
                                        return
                                    end
                                end
                            end
                    ...
                    if nww > 0
                        pos(w,:) = obj.inputXYZT( resx(ispan,w) , 1:obj.dim);
                        if obj.minTrackLength > 0
                            nvalid(w) = nvalid(w) + 1;
                        end
                    end
                    %go back and add obj.minTrackLength keyword thing
                    ...
                    if (nnew > 0) % & another keyword to workout inipos
                        ...
                        pos = [pos;obj.inputXYZT(eyes(newguys),1:obj.dim)];
                        nmem = zeros(nnew,1);
                        mem = [mem;nmem];
                        nun = 1:nnew;
                        ...
                        % put in obj.minTrackLength
                        n = n + nnew;
                    end
                    
                else
                    obj.logger.warn('No positions found for t=');
                end
                w = find( resx(ispan,:) ~= -1);
                nok = length(w);
                if nok ~= 0
                ...
                wlost = find(mem == obj.mem+1);
                nlost =length(wlost);
                
                if nlost > 0
                    pos(wlost,:) = -obj.maxdisp;
                    if obj.minTrackLength > 0
                        wdump = find(nvalid(wlost) < obj.minTrackLength);
                        ndump = length(wdump);
                        if ndump > 0
                            dumphash(wlost(wdump)) = 1;
                        end
                    end
                    % put in obj.minTrackLength keyword stuff if
                end
                if (ispan == zspan) || (i == z)
                    ...
                    obj.logger.info(i, ' of ' ,z, ' done.  Tracking ',ntrk,' particles ', n,' tracks total');

                    bigresx(i-(ispan)+1:i,:) = resx(1:ispan,:);
                    resx = zeros(zspan,n) - 1;
                    
                    
                    wpull = find(pos(:,1) == -obj.maxdisp);
                    npull = length(wpull);
                    ...
                    nkeep = length(wkeep);
                    if nkeep == 0
                        obj.logger.fatal('Were going to crash now, no particles....');
                    end
                    resx = resx(:,wkeep);
                    bigresx = bigresx(:,wkeep);
                    pos = pos(wkeep,:);
            ...
            if obj.minTrackLength > 0
                ...
                wkeep = find(nvalid >= obj.minTrackLength);
                nkeep = length(wkeep);
                if nkeep == 0
                    obj.logger.fatal('You are not going any further, check your params and data');
                end
                if nkeep < n
                    bigresx = bigresx(:,wkeep);
                    uniqid = uniqid(wkeep);
            ...
            wpull = find( pos(:,1) ~= -2*obj.maxdisp);
            npull = length(wpull);
            if npull > 0
                lillist = zeros(1,2);
                for ipull=1:npull
            ...
            tracks = zeros(nolist,dd+1);
            for j=1:dd
                tracks(:,j) = obj.inputXYZT(olist(:,1),j);
            end
            tracks(:,dd+1) = olist(:,2);
            
            ndat=length(tracks(1,:));
            
            indices = find(tracks(:,ndat) ~= circshift(tracks(:,ndat),-1));
            count = length(indices);
            if count > 0
                u = indices;
            else
                u = length(tracks(:,ndat)) -1;
            end

            ntracks=length(u);
            u=[0;u];
            for i=2:ntracks+1
                tracks(u(i-1)+1:u(i),ndat) = i-1;
            end
            obj.outputXYZTI = tracks;
        end
    end
end
