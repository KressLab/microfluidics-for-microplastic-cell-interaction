...
if nargin < 3
    l.error('--trackCloseGapsKalmanSparse: Incorrect number of input arguments!');
    errFlag = 1;
    return
end
...
if nargin < 5 || isempty(probDim)
    ...
    if probDim == 3 && probDimT == 2
        l.error('--trackCloseGapsKalmanSparse: Inconsistency in input. Problem 3D but no z-coordinates.');
        errFlag = 1;
    end
end
...
if errFlag
    l.error('--trackCloseGapsKalmanSparse: Please fix input parameters.');
    return
end
...
if numFramesEff == 0
    l.info('Empty movie. Nothing to track.');
    return
end
...
if selfAdaptive
    ...
    if verbose
        l.info('Linking features forwards ...');
    end
    ...
    if verbose
        l.info('Linking features backwards ...');
    end
    [~,~,kalmanInfoLink,~,linkingCosts] = linkFeaturesKalmanSparse(...
...
if any(trackStartTime > 1) || any(trackEndTime < numFramesEff)
    if verbose
        l.info(sprintf('Closing gaps (%d starts and %d ends) ...',...
